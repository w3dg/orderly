...
const DeleteButton = ({ id }: { id: number }) => {
  ...
  const deleteProduct = async (id: number) => {
    ...
    try {
      const { data } = await axios.delete(`/api/categories?cid=${id}`);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This will delete the category from the store. Any products in this category will be affected if still
            present
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
